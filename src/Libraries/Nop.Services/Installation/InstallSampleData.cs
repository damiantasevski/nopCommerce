...
public partial class InstallationService
{
    ...
    /// A task that represents the asynchronous operation
    /// </returns>
    protected virtual async Task InstallSampleCustomersAsync(List<SampleCustomer> jsonData)
    {
        ...
        var addresses = await jsonData.SelectAwait(async c => await createAddress(c)).ToListAsync();
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallCheckoutAttributesAsync(List<SampleCheckoutAttribute> jsonData)
    {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallSpecificationAttributesAsync(SampleSpecificationAttributes jsonData)
    {
        var attributes = jsonData.Attributes.ToDictionary(data => data,
             data => new SpecificationAttribute { Name = data.Name, DisplayOrder = data.DisplayOrder });

        foreach (var jsonModel in jsonData.AttributeGroups)
        {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallProductAttributesAsync(IList<SampleProductAttribute> jsonData)
    {
        await _dataProvider.BulkInsertEntitiesAsync(jsonData.Select(p => new ProductAttribute
        {
            Name = p.Name
        }));
    }
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallCategoriesAsync(IList<SampleCategory> jsonData)
    {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallManufacturersAsync(IList<SampleManufacturer> jsonData)
    {
        ...
        var allManufacturers = await jsonData.SelectAwait(async sample => await createManufacturer(sample)).ToListAsync();
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallForumsAsync(List<SampleForumGroup> jsonData)
    {
        foreach (var sampleForumGroup in jsonData)
        {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallDiscountsAsync(IList<SampleDiscount> jsonData)
    {
        await _dataProvider.BulkInsertEntitiesAsync(jsonData.Select(sd => new Discount
        {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallBlogPostsAsync(IList<SampleBlogPost> jsonData)
    {
        var languageId = await GetDefaultLanguageIdAsync();
        var customerId = await GetDefaultCustomerIdAsync();
        var storeId = await GetDefaultStoreIdAsync();

        var blogPosts = jsonData.ToDictionary(sbp => new BlogPost
        {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallNewsAsync(IList<SampleNewsItem> jsonData)
    {
        var languageId = await GetDefaultLanguageIdAsync();
        var customerId = await GetDefaultCustomerIdAsync();
        var storeId = await GetDefaultStoreIdAsync();

        var news = jsonData.ToDictionary(sni => new NewsItem
        {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallPollsAsync(IList<SamplePoll> jsonData)
    {
        var languageId = await GetDefaultLanguageIdAsync();

        var polls = jsonData.ToDictionary(sp => new Poll
        {
            LanguageId = languageId,
            Name = sp.Name,
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallWarehousesAsync(IList<SampleWarehouse> jsonData)
    {
        var warehouses = await jsonData.ToDictionaryAwaitAsync(
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallVendorsAsync(IList<SampleVendor> jsonData)
    {
        var vendors = jsonData.Select(sv => new Vendor
        {
            Name = sv.Name,
            Email = sv.Email,
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallAffiliatesAsync(IList<SampleAffiliate> jsonData)
    {
        var affiliates = await jsonData.ToDictionaryAwaitAsync(
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallOrdersAsync(IList<SampleOrder> jsonData)
    {
        var languageId = await GetDefaultLanguageIdAsync();
        ...
        var orders = await jsonData.ToDictionaryAwaitAsync(async so =>
        {
        ...
        var items = (await orders
            .SelectAwait(async o => await o.Value.OrderItems.SelectAwait(async oi => new KeyValuePair<OrderItem, SampleOrder.SampleOrderItem>(new OrderItem
            {
                ...
                OrderId = o.Key.Id,
                ProductId =
                    await GetFirstEntityIdAsync<Product>(p => p.Name == oi.ProductName) ??
                    throw new Exception($"Product with name \"{oi.ProductName}\" cannot be loaded"),
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallActivityLogsAsync(IList<SampleActivityLog> jsonData)
    {
        var customerId = await GetDefaultCustomerIdAsync();

        await _dataProvider.BulkInsertEntitiesAsync(await jsonData.SelectAwait(async sal => new ActivityLog
        {
    ...
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    protected virtual async Task InstallSearchTermsAsync(IList<SampleSearchTerm> jsonData)
    {
        var storeId = await GetDefaultStoreIdAsync();

        await _dataProvider.BulkInsertEntitiesAsync(jsonData.Select(sst => new SearchTerm
        {
            Count = sst.Count,
            Keyword = sst.Keyword,
            StoreId = storeId
